...
async function up(db) {
    await db.schema
        ...
        .addColumn('template_id', 'uuid', (col) => col.references('templates.id').onDelete('cascade'))
        .addColumn('type', 'varchar', (col) => col.notNull())
        .addColumn('workspace_id', 'uuid', (col) => col.references('workspaces.id').onDelete('cascade').notNull())
        .addColumn('created_at', 'timestamptz', (col) => col.defaultTo((0, kysely_1.sql) `now()`).notNull())
        .execute();
    await db.schema
...
//# sourceMappingURL=20260412T162318-favorites.js.map