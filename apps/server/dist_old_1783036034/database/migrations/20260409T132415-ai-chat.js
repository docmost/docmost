...
async function up(db) {
    ...
    await db.schema
        ...
        .ifNotExists()
        .addColumn('id', 'uuid', (col) => col.primaryKey().defaultTo((0, kysely_1.sql) `gen_uuid_v7()`))
        .addColumn('chat_id', 'uuid', (col) => col.references('ai_chats.id').onDelete('cascade').notNull())
        .addColumn('workspace_id', 'uuid', (col) => col.references('workspaces.id').onDelete('cascade').notNull())
...
async function down(db) {
    ...
    await (0, kysely_1.sql) `DROP TRIGGER IF EXISTS ai_chat_messages_tsvector_update ON ai_chat_messages`.execute(db);
    await (0, kysely_1.sql) `DROP FUNCTION IF EXISTS ai_chat_messages_tsvector_trigger`.execute(db);
    await db.schema.dropTable('ai_chat_messages').execute();
    await db.schema.dropTable('ai_chats').execute();
}
//# sourceMappingURL=20260409T132415-ai-chat.js.map