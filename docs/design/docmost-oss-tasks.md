...
- ✅ D5 Agent Skill bundle: `skills/docmost.skills.json` (18 skills) + `RECIPE.{organize,code-to-wiki}.md` + `skills/docmost/SKILL.md` (openclaw) + `README.md`; **MCP tools added** (`list_labels`/`add_page_labels`/`set_page_summary`/`dedup_analyze`/`organize_create|report|close`, 9 MCP tests, build+lint green). Optional left: filtered OpenAPI doc (from running Swagger), per-skill openclaw split (A3 headline + h as recipe)
