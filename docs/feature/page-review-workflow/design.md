...
**Verified against real schema — action required:** `apps/server/src/database/migrations/20260413T121647-page-verifications.ts:10` defines `page_id` as `.notNull().unique().references('pages.id')`. This unique constraint makes the multi-row-per-page model impossible as-is. The Task 1 migration (§3.1 below, updated) MUST `dropConstraint` (or `dropIndex`, whichever Postgres actually created for `.unique()` here — verify with `\d page_verifications` before writing `down()`) on that column before this design is buildable, replacing it with a plain (non-unique) index on `page_id` for lookup performance.

**Consequence for existing read paths:** `PageVerificationRepo.findByPageId` (`page-verification.repo.ts:19`) has no `ORDER BY` and is called from six places in `page-verification.service.ts` (`setup`, `update`, `remove`, `verify`, `submitForApproval`/`rejectApproval` equivalents, `markObsolete`). Once a page can have >1 row, this method's result becomes non-deterministic. Resolution: every one of the new review-flow methods (`submit`, `approve`, `reject`, `requestClarification`, `getReviewPayload`, and the private `getInApprovalVerificationForVerifier` helper) MUST call `findLatestByPageId` (ordered `createdAt desc`, §4), never `findByPageId`. The pre-existing `type: 'expiring'` methods (`setup`'s uniqueness check, `update`, `remove`) keep using `findByPageId` unmodified — they still only ever see one row per page, since the multi-row case is exclusive to `type: 'qms'` cycles created by the content-change listener.
...
**Correction from earlier draft:** `VerificationNotificationService` (`processApprovalRequested`, `processApprovalRejected`, etc.) is NOT an EE service — it lives at `apps/server/src/core/notification/services/verification.notification.ts` and is registered in the core `NotificationModule`. This is pre-existing structure (the original QMS approval feature was built directly in core, before this project's golden rule was written) — extending it further would compound the violation, not follow precedent.

Reuse the existing queue job names `PAGE_APPROVAL_REQUESTED_NOTIFICATION`, `PAGE_APPROVAL_REJECTED_NOTIFICATION` as-is (their handlers already exist in core; not touched by this design). For the two new job types, all notification-composition logic goes into a **new EE service**:

- Create `apps/server/src/ee/page-verification/services/review-notification.service.ts`, `ReviewNotificationService`, with `processApprovalClarification(job, appUrl)` and `processReverificationRequired(job, appUrl)` — copy the shape of `VerificationNotificationService.processApprovalRejected` (in-app notification insert + transactional email) but as new EE-owned code, not additions to the core file.
- Register `ReviewNotificationService` as a provider in `page-verification.module.ts`.
- **Core touch uses an existing precedent, not a new pattern**: `notification.processor.ts` already has `runVerificationReconcile()` (lines ~181-204) — a private method that resolves an EE service *without* NotificationModule statically importing anything under `ee/`: it `require()`s the EE file inside a `try/catch` (falling back to a no-op debug log if EE isn't bundled), then calls `this.moduleRef.get(EeClass, { strict: false })` (a `ModuleRef` already injected in the constructor) to fetch the live instance from Nest's DI container. This matters because `NotificationModule` is a core module loaded unconditionally at boot (unlike `app.module.ts`'s top-level `require('./ee/ee.module')` inside a try/catch) — a static `import { ReviewNotificationService } from '.../ee/page-verification/...'` here would break the documented guarantee that "the app still boots if `ee/` is missing" (`CLAUDE.md`). Add two private methods, `runApprovalClarification(job)` / `runReverificationRequired(job)`, mirroring `runVerificationReconcile`'s require+moduleRef.get shape exactly, and call them from two new `case` lines in `process()`. No new import statement lands in `notification.processor.ts` at all — the require is dynamic, same as the existing method.

This keeps every line of *what the notification says and to whom* inside `ee/page-verification`, and keeps core's EE-optionality guarantee intact — core's only job is a two-method addition that exactly replicates an already-reviewed, already-shipped pattern in the same file.
...
2. Previous verifiers ARE notified on silent revert-to-draft (§4 step 3) via a new `PAGE_REVERIFICATION_REQUIRED_NOTIFICATION` job, handled by the new EE `ReviewNotificationService` (§7) — core only gains one routing case, not new business logic.
...
4. **Self-review correction:** the original draft understated two things that are now fixed above — (a) `page_verifications.page_id` has a DB `UNIQUE` constraint that must be dropped in the Task 1 migration for the multi-row-per-page model to work at all (§3.1); (b) the plan had briefly proposed adding the two new notification methods directly to the core `VerificationNotificationService` — corrected to a new EE `ReviewNotificationService` (§7) so no new business logic lands in core.
